...
const BOT_TOKEN = process.env.BOT_TOKEN || '8728397123:AAH7SGg0CBGLHds2QSMxps0F1FkCIvlmbvM';
...
const SETTINGS_DATA_KEY = 'ezpaySettings';
const PRIMARY_DATA_KEY = 'ezpayData';
...
async function resolveTokenAndUser(input) {
  ...
  // Token-shape detection: real tokens are long base64-ish
  ...
  if (isPhone) {
    ...
    // Fallback to in-memory trackedUsers
    if (!found) {
      try {
...
/addbank Rahul Kumar|1234567890|SBIN0001234|SBI|rahul@upi`
    ...
    if (tokenCmdMatch) {
      ...
      // Headers exactly as APK sends them
